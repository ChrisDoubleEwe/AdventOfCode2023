...
#   - Remove 1 brick
#   - Run the falling algorithm, keeping note of which bricks move

brick_index = -1
brick_copy = copy.deepcopy(bricks)
this_bricks = copy.deepcopy(bricks)
partb = 0;
for b in bricks:
  moved_bricks = []
  brick_index += 1
  print "Removing brick " + str(brick_index)
  this_bricks = copy.deepcopy(bricks)
  this_bricks.pop(brick_index)
  ...
  while falling == 1:
    ...
    for b in this_bricks:
      ...
      if highest_support+1 < bz_start:
        brick_height = bz_end - bz_start
        this_bricks[bindex][0][2] = highest_support+1
        this_bricks[bindex][1][2] = highest_support+1+brick_height
        if bindex not in moved_bricks:
          moved_bricks.append(bindex)
    if (len(moved_bricks)) != last_num_bricks:
      falling = 1
      last_num_bricks = len(moved_bricks)
    else:
      falling = 0
  print "  " + str(len(moved_bricks)) + " moved"
  partb += len(moved_bricks)
